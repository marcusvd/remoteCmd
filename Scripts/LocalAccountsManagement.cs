...
public static class LocalAccountsManagement
{
    ...
    private static int Flags(DirectoryEntry userEntry, bool passwordExpires, bool neverExpires, string userName)
    {
        int userFlags = (int)userEntry.InvokeGet("UserFlags");
        if (OperatingSystem.IsWindows())
        {
            // Console.WriteLine($"UserFlags before: {userFlags:X}");

            if (passwordExpires)
            {
                // Set the flag to expire the password at the next logon.
                userFlags |= 0x80000; // ADS_UF_PASSWORD_EXPIRED
                Tools.ProcessExecutorCmdLine("net", $"user {userName} /logonpasswordchg:yes", BasePath.AppSettingsJsonFile);
            }
            else
                // Remove the password expiration flag if it is set.
                userFlags &= ~0x80000;

            if (neverExpires)
                // Set the flag to never expire the password.
                userFlags |= 0x10000; // ADS_UF_DONT_EXPIRE_PASSWD
            else
                // Remove the "password never expires" flag if it is set.
                userFlags &= ~0x10000;

            // Update the UserFlags in Active Directory.
    ...
    public static void ChangePassword(string userName, string password, bool passwordExpires = false, bool neverExpires = false)
    {
        try
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                ...
                userEntry.Invoke("Put", new object[] { "UserFlags", Flags(userEntry, passwordExpires, neverExpires, userName) });
    ...
    public static void CreateLocalAccount(string userName, string password, string[] groups, bool passwordExpires = false, bool neverExpires = false)
    {
        try
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                ...
                userEntryAgain.Invoke("Put", new object[] { "UserFlags", Flags(userEntryAgain, passwordExpires, neverExpires, userName) });
